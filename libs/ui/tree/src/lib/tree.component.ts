...
export class TreeComponent implements AfterViewInit {
  ...
  private _data!: Array<TreeData>;
  private tree!: d3.Selection<SVGGElement, unknown, HTMLElement, unknown>;
  private information!: d3.HierarchyPointNode<unknown>;

  // todo should handle data update (though do we need that?)
  @Input() public set data(value: Array<TreeData>) {
  ...
  private convertDataIntoD3Information(): void {
    const dataStructure = d3
      .stratify()
      .id((data) => (<TreeData>data).id)
      .parentId((data) => (<TreeData>data).parentId)(this._data);
  ...
  private drawConnections(): void {
    ...
    connections
      .append('text')
      .text((data) => (<TreeData>data.target.data).townId ?? '')
  ...
  private drawNodes(): void {
    ...
    nodes
      .append('text')
      .text((data) => (<TreeData>data.data)?.displayId ?? 'x')
      .attr('x', (data) => data.x)
    ...
    nodes
      .append('text')
      .text((data) => (<TreeData>data.data)?.cost ?? '')
